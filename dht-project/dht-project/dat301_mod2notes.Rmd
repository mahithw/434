...
Control Structures (“blocks”) allow you to control the flow of execution of the program, depending on appropriate condiditons.
...
Some of the frequently used argument to help the visual interface are:  `main` to give the title, `xlab` and `ylab` to provide labels for the axes, n`ames.arg` for naming each bar, `col` to define color etc.  We can also plot bars horizontally by providing the argument `horiz = TRUE`.  Argument `density =` will shade the bars.
...
titanic <- apply(Titanic, c(4, 1), sum)
```
...
*Exercise*:
Using `cars`VADeaths` dataset, plot the death rates of Female Urbans using the age group as the x axis and the height representing the death rate.  You may add axes and titles and change coloring if you wish.
...
The binomial distribution describes the probability getting a certain numer of successes (k) from a set number of trials (n) given a probability for success (p).
...
## distribution is right skewed since we have smaller probability of this occuring
...
pie(exp,
    ...
    col = c("white", "grey", "black", "darkblue", "blue"),
    border = "darkmagenta",
    clockwise = TRUE)
```
...
The expected value is is a generalization of the weighted average, and is intuitively the arithmetic mean of a large number of independent realizations of X.
...
```{r}
x <- c(rep(10, 10), rep(15, 12), rep(9, 2), rep(8, 8))
xbar <- mean(x); xbar
s2 <- var(x); s2
s <- sqrt(var(x)); s
s <- sd(x); s
...
##### Notes
Boxplots show the variation/spread of datasets.  
...
Note: 
When splitting the two data in half and n is odd, there are two methods: inclusive (include median 2x) and exclusive (omit the median).
...
• If the median is to the left of the center ofthe boxplot, the data is skewed right
...
Some of the frequently used parameters are: `main` to give the title, `xlab` and `ylab` to provide labels for the axes, `col` to define color etc.  Additionally, with the argument `horizontal = TRUE` we can plot the boxplot horizontally and with `notch = TRUE` we can add a notch to the box.


*Example*:
Create boxplot using the ozone variable from `airquality` data.
...
##### Computing quartiles by hand with quantile() function
Syntax:
```{r eval=FALSE}
...
## set na.rm = TRUE to remove the missing data

quantile(ozone, probs = seq(0, 1, by=0.25), na.rm = TRUE)
```
...
*Example*:
For example, in our dataset `airquality`, the `Solar.R` can be our numeric vector. `Month` can be our grouping variable, so that we get the boxplot for each month separately. In our dataset, month is in the form of number (1=January, 2-Febuary and so on).  Also included a legend using the `legend()` command after the close of the `boxplot()` command.

```{r}
boxplot(Solar.R~Month, data=air, horizontal=TRUE, 
        col=c("darkblue", "blue", "darkgreen", "green", "white"))
...
*Exercise*
From `ToothGrowth` df can you make a stacked boxplot for length of tooth growth by supplement type?  May change colors or add title etc. at your own discretion.
...
We can use funciton `lines()` to add a curve, `abline()` for a straight line, or `points()` for adding scatterplot.
...
Density formulas:
For X~N(mu, sigma^2):  f(x) = 1/(sqrt(2(pi)(sigma^2))) * e^((-x-mu)^2/(2(pi)(sigma^2)))

For Z~N(0,1):  f(x) = 1/sqrt(2pi) * e^(-x^2/2)

Function to create random numbers from a normal distribution:
...
hist(mtcars$mpg, 
     breaks = 12, 
     freq = FALSE,
     col = rgb(red = 1, gren = 0, blue = 1, alpha = 0.1))
```
...
Apply-family is a family of functions in base R used for data manipulation.
Notes from Johns Hopkins: http://github.com/DataScienceSpecialization/courses/tree/master/02_RProgramming

Used when you want to evaluate a function over the margins of an array.
...
```{r}
m <- matrix(1:9, nrow=3, byrow=TRUE); m[2, 2] <- -1
m

## remember matrix has 2 dimensions - first dimension is row, second is column
...
```{r}
x <- list(a = 1:4, b = rnorm(10), c = rnorm(20, 1), d = rnorm(100, 5)); x
